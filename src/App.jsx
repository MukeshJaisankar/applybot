...
function JDInput({ onSubmit, disabled }) {
  ...
  return (
    <div className="card">
      ...
      <textarea className="textarea" placeholder="Paste the full job description here..." value={jdText} onChange={(e) => setJdText(e.target.value)} disabled={disabled} rows={10} />
      <label className="label">Job Posting Link</label>
      <input className="input" type="url" placeholder="https://company.com/careers/job-id" value={jdLink} onChange={(e) => setJdLink(e.target.value)} disabled={disabled} />
      <label className="label">Where did you find this?</label>
      <select className="input" value={source} onChange={(e) => setSource(e.target.value)} disabled={disabled}>
    ...
    <div className="app">
      <header className="header">
        <div className="header-inner">
          <div className="logo"><span className="logo-icon">◈</span><span className="logo-text">ApplyBot</span></div>
          <div className="header-meta">Mukesh Jaisankar · Job Application Automator</div>
        </div>
      </header>
      <main className="main">
        <div className="hero">
          <h1 className="hero-title">Paste JD.<br /><span className="accent">We handle the rest.</span></h1>
          <p className="hero-sub">Finds recruiters · Writes your email · Sends on schedule</p>
        </div>
